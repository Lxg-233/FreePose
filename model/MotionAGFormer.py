...
# from model.me.drop import DropPath
...
from model.me.TTE2_corre import EnhancedTTEBlock
# model.me.TTE_nofft 原始
...
class MotionAGFormerBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., act_layer=nn.GELU, attn_drop=0., drop=0., drop_path=0.,
        ...
        #                                   neighbour_num=neighbour_num,
        #                                   n_frames=n_frames)
        self.att_temporal = EnhancedTTEBlock(dim, num_heads, mlp_hidden_dim=dim, qkv_bias=qkv_bias, qk_scale=qk_scale, )
    ...
    def forward(self, x):
        ...
        # yes
        x_spatial = self.att_spatial(self.graph_spatial(x))
        x_temporal = self.att_temporal(x)
        alpha = torch.cat((x_spatial, x_temporal), dim=-1)
        alpha = self.fusion(alpha)
        alpha = alpha.softmax(dim=-1)
        x = x_spatial * alpha[..., 0:1] + x_temporal * alpha[..., 1:2]
...
class MotionAGFormer(nn.Module):
    ...
    def __init__(self, n_layers, dim_in, dim_feat, dim_rep=512, dim_out=3, mlp_ratio=4, act_layer=nn.GELU, attn_drop=0.,
        ...
        # self.temp_embed = nn.Parameter(torch.zeros(1, n_frames, 1, dim_feat))  # ------new------------
        # trunc_normal_(self.temp_embed, std=.02)
        trunc_normal_(self.pos_embed, std=.02)  # new----------------------------------   ----
...
def _test():
    ...
    print(f"Model FLOPS #: {profile_macs(model, random_x):,}")  # MACs
