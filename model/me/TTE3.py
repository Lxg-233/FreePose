...
class EnhancedTTEBlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_hidden_dim,
        ...
        # 将 learnable_filter 改为 nn.Linear
        self.learnable_filter = nn.Linear(dim, dim)  # 矩阵变换
    ...
    def forward(self, x):
       ...
       # fft
        ...
        gate = self.high_gate(high) # BJ C T/2-->gate:BJ C T/2

        gate = gate.transpose(1, 2)  # 形状变为 (B, T, C)
        gate = self.learnable_filter(gate)  # 形状变为 (B, T, C)
        gate = gate.transpose(1, 2)  # 形状变为 (B, C, T)
